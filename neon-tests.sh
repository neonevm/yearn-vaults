...
docker run --rm --network host -e MNEMONIC_PHRASE="$MNEMONIC_PHRASE" \
  ...
  bash -c "brownie run scripts/neon_faucet.py && brownie test tests/functional/vault/test_permit.py::test_permit -v --network neon && brownie test tests/functional/strategy/test_strategy_health.py -v --network neon"
